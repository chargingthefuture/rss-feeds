#!/usr/bin/env node
/**
 * Collects a YouTube channel's upload list into channels/<slug>.json.
 *
 ...
 * it in the repository, where `pnpm build` turns it into a feed.
 *
 ...
 *
 * Usage:
 *   pnpm collect -- --channel @SomeChannel
 *   pnpm collect -- --channel @SomeChannel --cutoff 2024-12-31
 *   pnpm collect                                   # refresh every archive
 *
...
function refresh(channelUrl: string, slug: string, existing: Archive | null, cutoff?: string): void {
  ...
  console.log(`✓ ${archive.name}: ${videos.length} videos (${added} new) → channels/${slug}.json`);
}
...
function main(): void {
  ...
  if (!existsSync(ARCHIVE_DIR)) {
    console.log('No channels/ yet, and no channel given. Nothing to do.');
    return;
  }
  const files = readdirSync(ARCHIVE_DIR).filter((name) => name.toLowerCase().endsWith('.json'));
  if (files.length === 0) {
    console.log('No archives in channels/, and no channel given. Nothing to do.');
    return;
  }
